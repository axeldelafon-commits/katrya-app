...
export default async function ProductDetailPage(
  ...
  const { data: tag } = await supabase.from('nfc_tags').select('*').eq('product_id', id).maybeSingle()
  const { data: latestPassport } = await supabase.from('passports').select('*').eq('product_id', id).order('version', { ascending: false }).limit(1).maybeSingle()
  const { data: events } = await supabase.from('events').select('*').eq('product_id', id).order('created_at', { ascending: false }).limit(20)

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
        <Link href="/admin/products" style={{ color: '#888', textDecoration: 'none', fontSize: 14 }}>← Retour</Link>
      ...
      <div style={grid2}>
        <div style={card}>
          ...
          <Link href={`/p/${product.katrya_id}`} target="_blank" style={{ color: '#0cf', fontSize: 13 }}>Voir passeport public ↗</Link>
        </div>
      ...
      <div style={card}>
        ...
        {events?.map((ev) => (
          <div key={ev.id} style={{ borderBottom: '1px solid #1a1a1a', paddingBottom: 12, marginBottom: 12 }}>
            <p style={{ margin: '0 0 4px' }}><strong>{ev.event_type}</strong> <span style={{ color: '#888', fontSize: 12 }}>— {ev.actor_type}</span></p>
            <p style={{ margin: '0 0 4px', fontSize: 12, color: '#666' }}>{new Date(ev.created_at).toLocaleString('fr-FR')}</p>
...
const card: React.CSSProperties = { border: '1px solid #222', borderRadius: 12, padding: 20 }
const grid2: React.CSSProperties = { display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 24 }
const preStyle: React.CSSProperties = { background: '#0d0d0d', padding: 8, borderRadius: 6, overflowX: 'auto', fontSize: 11, margin: 0 }
